...
Adding 1 + 2 in a new box takes 3 seconds.Adding 3 + 3 in a new box takes 6 seconds.
Adding 4 + 6 in a new box takes 10 seconds.Adding 5 + 10 in a new box takes 15 seconds.
Hence total time taken is 34 seconds. 
...
for i in range(t):
    ...
    while len(arr)!=1:                                      # While loop will run untill len(arr) becomes 1
        ...
        arr[1]=arr[0]+arr[1]                                # Because of sorting we are sure tha the first 2 elements of arr are the two smallest ones. So we add them and store the result in arr[1] only.
